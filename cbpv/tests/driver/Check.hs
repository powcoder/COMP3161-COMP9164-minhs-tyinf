...
getSkips skips = concat <$> (foreach skips $ \skip -> map (<.> ".hst") . map (takeDirectory skip </>) . lines <$> readFile skip)

runTests exe testdir = do
    files <- traverseP $ testdir
    let tests' = filter ((".hst" ==) . takeExtension) files
    let skips = filter (("Skip" ==) . takeBaseName) files
    tests <- (tests' \\) <$> getSkips skips
    marks <- foreach (sort tests) (\test -> do
      ...
      case exit of
        ExitFailure i -> if expect_fail then check
                                 ...
                                 dumpOutput err out testContents
  ...
  where
    dumpOutput err out testContents = do
      putStrLn "Input code was:"
